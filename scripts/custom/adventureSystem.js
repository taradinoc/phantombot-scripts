...
(function() {
    ...
    function loadStories() {
        ...
        currentAdventure.unrevealedSurvivors = [];
        currentAdventure.caught = [];
        currentAdventure.unrevealedCaught = [];
        currentAdventure.gameState = 0;
        currentAdventure.gainPercent = gainPercent;
    ...
    function loadStoriesFromPrefix(prefix) {
        var storyId = 1,
         ...
         for (storyId; $.lang.exists(prefix + '.' + storyId + '.title'); storyId++) {
            ...
            gain = $.lang.exists(prefix + '.' + storyId + '.gain') ? parseInt($.lang.get(prefix + '.' + storyId + '.gain')) : null;
    ...
    function calculateResult() {
        ...
        for (i in currentAdventure.users) {
            if ($.randRange(0, 20) > 5) {
                ...
                currentAdventure.unrevealedSurvivors.push(currentAdventure.users[i]);
            } else {
                currentAdventure.caught.push(currentAdventure.users[i]);
                currentAdventure.unrevealedCaught.push(currentAdventure.users[i]);
            }
        }
    };
     ...
     * @returns {string}
     */
    function revealNext(array) {
        var i = $.randRange(0, array.length - 1);
        var selected = array.splice(i, 1)[0];
        return $.username.resolve(selected.username);
    }
    ...
    function replaceTags(line) {
        if (reProb.test(line)) {
            var prob = parseInt(RegExp.$1);
            if ($.rand(100) >= prob) {
                return '';
            } else {
                line = line.replace(reProb, '');
            }
        }
        ...
        if (line.indexOf('(next caught)') > -1) {
            if (currentAdventure.unrevealedCaught.length > 0) {
                line = $.replace(line, '(next caught)', revealNext(currentAdventure.unrevealedCaught));
            } else {
                return '';
            }
        }
        if (line.indexOf('(next survivor)') > -1) {
            if (currentAdventure.unrevealedSurvivors.length > 0) {
                line = $.replace(line, '(next survivor)', revealNext(currentAdventure.unrevealedSurvivors));
            } else {
                return '';
            }
        }
        if (line.indexOf('(other caught)') > -1) {
            if (currentAdventure.unrevealedCaught.length > 0) {
                line = line.replace('(other caught)', adventureUsersListJoin(currentAdventure.unrevealedCaught));
                currentAdventure.unrevealedCaught = [];
            } else {
                return '';
            }
        }
        if (line.indexOf('(other survivors)') > -1) {
            if (currentAdventure.unrevealedSurvivors.length > 0) {
                line = line.replace('(other survivors)', adventureUsersListJoin(currentAdventure.unrevealedSurvivors));
                currentAdventure.unrevealedSurvivors = [];
            } else {
                return '';
            }
        }
        if (reGainPlus.test(line)) {
            currentAdventure.gainPercent += parseInt(RegExp.$1);
            line = line.replace(reGainPlus, '');
        }
        return line;
    };
    ...
    function runStory() {
            ...
            if (stories[i].game != null) {
                if (game.equalsIgnoreCase(stories[i].game)) {
                    ...
                    temp.push({
                        ...
                        gainPercent: stories[i].gainPercent
                    });
                }
            } else {
        ...
        } while (story.index === lastStory && stories.length != 1);

        lastStory = story.index;

        if (typeof story.gainPercent === 'number') {
            currentAdventure.gainPercent = story.gainPercent;
        }
    ...
    function endHeist() {
        ...
        for (i in currentAdventure.survivors) {
            pay = (currentAdventure.survivors[i].bet * (currentAdventure.gainPercent / 100));
    ...
    $.bind('initReady', function() {
        $.registerChatCommand('./custom/adventureSystem.js', 'adventure', 7);
